...
func Test_Get_Repository(t *testing.T) {
	...
	t.Run("Happy Path - Retorna genre", func(t *testing.T) {
		setup := setupRepository(t)
		repository := NewRepository()
		now := time.Now().UTC().Format(time.RFC3339)
		expected := entity.Genre{Name: "Ficção", Description: strPtr("Literatura ficcional")}
		expected.Id = 1
		rows := sqlmock.NewRows([]string{"id", "name", "description", "created_at", "updated_at"}).AddRow(expected.Id,
			expected.Name,
	...
	t.Run("Error Path - Banco retorna erro", func(t *testing.T) {
		...
		assert.Equal(t, setup.ctx, returnedCtx)
		assert.Equal(t, entity.Genre{}, result)
		assert.NoError(t, setup.sqlMock.ExpectationsWereMet())
	})
}
...
func Test_Create_Repository(t *testing.T) {
	...
	t.Run("Happy Path - Cria genre", func(t *testing.T) {
		setup := setupRepository(t)
		repository := NewRepository()
		input := entity.Genre{Name: "Ficção", Description: strPtr("Literatura ficcional")}
		expected := input
		expected.Id = 1
		now := time.Now().UTC().Format(time.RFC3339)
		rows := sqlmock.NewRows([]string{"id", "name", "description", "created_at", "updated_at"}).AddRow(expected.Id,
		...
		tx, err := setup.db.Beginx()
		assert.NoError(t, err)
		setup.sqlMock.ExpectQuery(regexp.QuoteMeta(createSql)).WithArgs(input.Name, input.Description).WillReturnRows(rows)
	...
	t.Run("Error Path - Banco retorna erro", func(t *testing.T) {
		...
		tx, err := setup.db.Beginx()
		assert.NoError(t, err)
		setup.sqlMock.ExpectQuery(regexp.QuoteMeta(createSql)).WithArgs(input.Name, input.Description).WillReturnError(expectedError)
		returnedCtx, err, result := repository.Create(setup.ctx, tx, input)
		assert.Error(t, err)
		assert.ErrorIs(t, err, expectedError)
		assert.Equal(t, setup.ctx, returnedCtx)
...
func Test_Update_Repository(t *testing.T) {
	...
	t.Run("Happy Path - Atualiza genre", func(t *testing.T) {
		setup := setupRepository(t)
		repository := NewRepository()
		id := 1
		input := entity.Genre{Name: "Ficção", Description: strPtr("Literatura ficcional")}
		...
		tx, err := setup.db.Beginx()
		assert.NoError(t, err)
		setup.sqlMock.ExpectQuery(regexp.QuoteMeta(updateSql)).WithArgs(input.Name, input.Description, id).WillReturnRows(rows)
	...
	t.Run("Error Path - Banco retorna erro", func(t *testing.T) {
		...
		tx, err := setup.db.Beginx()
		assert.NoError(t, err)
		setup.sqlMock.ExpectQuery(regexp.QuoteMeta(updateSql)).WithArgs(input.Name, input.Description, id).WillReturnError(expectedError)
		returnedCtx, err, result := repository.Update(setup.ctx, tx, id, input)
		assert.Error(t, err)
		assert.ErrorIs(t, err, expectedError)
		assert.Equal(t, setup.ctx, returnedCtx)
...
func Test_Delete_Repository(t *testing.T) {
	...
	t.Run("Happy Path - Exclui genre", func(t *testing.T) {
		setup := setupRepository(t)
		repository := NewRepository()
		id := 1
		expected := entity.Genre{Name: "Ficção", Description: strPtr("Literatura ficcional")}
...
func Test_List_Repository(t *testing.T) {
	...
	t.Run("Happy Path - Retorna lista", func(t *testing.T) {
		setup := setupRepository(t)
		repository := NewRepository()
		input := entity.GenreFilters{Offset: 0, Limit: 10}
		now := time.Now().UTC().Format(time.RFC3339)
		expected := entity.Genre{Name: "Ficção", Description: strPtr("Literatura ficcional")}
		expected.Id = 1
		rows := sqlmock.NewRows([]string{"id", "name", "description", "created_at", "updated_at"}).AddRow(expected.Id,
	...
	t.Run("Error Path - Banco retorna erro", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, expectedError)
		assert.Equal(t, setup.ctx, returnedCtx)
		assert.Equal(t, entity.GenreList{}, result)
		assert.NoError(t, setup.sqlMock.ExpectationsWereMet())
	})
}
