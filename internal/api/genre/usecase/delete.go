...
func NewDeleteUC(db *sqlx.DB, repo genre.Repository, redisCli *redis.Client) DeleteUC {
...
func (u *DeleteUC) Execute(ctx context.Context, id int, claims *entity.AuthClaims) (returnedCtx context.Context, err error) {
	...
	_ = u.redisCli.Del(ctx, fmt.Sprintf("biblioteca_genre_get_%d", id), keyList).Err()
