...
func NewUpdateUC(db *sqlx.DB, repo book.Repository, redisCli *redis.Client) UpdateUC {
	return UpdateUC{db: db, repo: repo, redisCli: redisCli}
}

func (u *UpdateUC) Execute(ctx context.Context, id int, input entity.Book, claims *entity.AuthClaims) (returnedCtx context.Context, err error, result entity.Book) {
	...
	u.saveRedis(ctx, result, fmt.Sprintf("biblioteca_book_get_%d", id))
	_ = u.redisCli.Del(ctx, keyList).Err()

	return
}

func (u *UpdateUC) saveRedis(ctx context.Context, result entity.Book, key string) {
