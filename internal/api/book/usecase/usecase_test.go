...
func Test_Get_UseCase(t *testing.T) {
	...
	t.Run("Error Path - Repository retorna erro", func(t *testing.T) {
		...
		s.sqlMock.ExpectBegin()
		s.repo.On("Get", mock.Anything, mock.Anything, 1).Return(s.ctx, x, entity.Book{}).Once()
...
func Test_List_UseCase(t *testing.T) {
	s := setup(t)
	in := entity.BookFilters{}
	expectedIn := in
	expectedIn.SetDefault()
	expected := entity.BookList{}
	s.sqlMock.ExpectBegin()
...
func Test_Create_UseCase_Validation(t *testing.T) {
	...
	_, e, r := s.uc.Create(s.ctx, entity.Book{}, claims)
	assert.Error(t, e)
	assert.Equal(t, entity.Book{}, r)
...
func Test_Update_UseCase_Validation(t *testing.T) {
	...
	s.sqlMock.ExpectBegin()
	claims := &entity.AuthClaims{Role: entity.RoleAdmin}
	_, e, r := s.uc.Update(s.ctx, 1, entity.Book{}, claims)
	assert.Error(t, e)
	assert.Equal(t, entity.Book{}, r)
