...
func NewHandler(bookUC book.UseCase) *Handler {
...
func (h *Handler) Create() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		ctx, err, resp := h.bookUC.Create(ctx, input, claims)
		if err != nil {
			return helpers.ResponseErrorHTTP(c, helpers.REPONSE_HTTP_BAD_REQUEST, err)
		}
...
func (h *Handler) Update() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		ctx, err, resp := h.bookUC.Update(ctx, id, input, claims)
		if err != nil {
			return helpers.ResponseErrorHTTP(c, helpers.REPONSE_HTTP_BAD_REQUEST, err)
		}
...
func (h *Handler) Delete() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		ctx, err = h.bookUC.Delete(ctx, id, claims)
