...
func Test_Create_Handler(t *testing.T) {
	...
	t.Run("Happy Path - Cria reserva", func(t *testing.T) {
		...
		position := 1
		expected := entity.Reservation{Id: 10, UserId: 1, BookId: 2, Position: &position, Status: entity.ReservationStatusWaiting}
		body, err := json.Marshal(input)
		assert.NoError(t, err)

		s.useCase.On("Create", mock.Anything, input).Return(s.ctx, nil, expected).Once()

		req := httptest.NewRequest(http.MethodPost, "/reservation/create", bytes.NewReader(body))
	...
	t.Run("Error Path - JSON inválido", func(t *testing.T) {
		s := setupHandler(t)
		req := httptest.NewRequest(http.MethodPost, "/reservation/create", bytes.NewBufferString(`{"user_id":`))
		req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
		response := httptest.NewRecorder()
		echoCtx := s.echo.NewContext(req, response)
		...
		s.useCase.AssertNotCalled(t, "Create")
	})
	...
	t.Run("Error Path - Use case retorna erro", func(t *testing.T) {
		...
		s.useCase.On("Create", mock.Anything, input).
