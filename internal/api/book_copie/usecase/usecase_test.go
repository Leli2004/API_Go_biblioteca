...
func Test_Get_UseCase(t *testing.T) {
	...
	t.Run("Error Path - Repository retorna erro", func(t *testing.T) {
		...
		s.sqlMock.ExpectBegin()
		s.repo.On("Get", mock.Anything, mock.Anything, 1).Return(s.ctx, x, entity.BookCopy{}).Once()
...
func Test_List_UseCase(t *testing.T) {
	s := setup(t)
	in := entity.BookCopyFilters{}
	expectedIn := in
	expectedIn.SetDefault()
	expected := entity.BookCopyList{}
	s.sqlMock.ExpectBegin()
