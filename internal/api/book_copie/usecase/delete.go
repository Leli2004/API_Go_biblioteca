...
func NewDeleteUC(db *sqlx.DB, repo book_copie.Repository, redisCli *redis.Client) DeleteUC {
...
func (u *DeleteUC) Execute(ctx context.Context, id int, claims *entity.AuthClaims) (returnedCtx context.Context, err error) {
	...
	_ = u.redisCli.Del(ctx, fmt.Sprintf("biblioteca_book_copie_get_%d", id), keyList).Err()
